...
exports.newSnakeRank =async(email, score) =>{
...
exports.getPongRank = async()=>{
    return new Promise((resolve, reject)=>{
        let sql = "select user.name, rank_pong.score from rank_pong join user on rank_pong.user_id = user.id order by score desc limit 10";
        conn.query(sql, function(error, result){
            return error? reject(error) : resolve(result)
        })
    })
...
exports.newPongRank =async(email, score) =>{

    return new Promise((resolve, reject)=>{
         //일단 rank 정보가 있는지 확인
        let params = [email];
        let sql = "select * from rank_pong join user on rank_pong.user_id = user.id where user.email =?"
        conn.query(sql,params,function(error, result){
            if(error) return reject(false);
            else{
                if(result.length===0){ //없으면 추가
                        ...
                        if(error) return reject(false);
                            ...
                            conn.query(sql, params, function(error, re){
                                if(error) return reject(false);
                                    ...
                                    console.log("pong_snake 추가 완료!")
                                    return resolve(true);
                                }
                            })
                        }
                    })
                }else{ //있으면 score 비교후 갱신할 지 결정
                    let max = result[0].score;     
                    let rank_score = Math.max(max, score);
